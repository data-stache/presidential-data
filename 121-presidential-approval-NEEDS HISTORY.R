...
ar_temp_2 <- read.csv('https://projects.fivethirtyeight.com/biden-approval-data/approval_polllist.csv', stringsAsFactors = FALSE, header = TRUE)
tail(ar_temp_2)
...
approval_ratings <- approval_ratings %>%
  ...
  mutate(net_approve = approve - disapprove,
         ...
         run_date = RUN_DATE) %>%
  arrange(desc(end_date)) %>%
  select(run_date, field_date, start_date, end_date, president, pollster, fte_grade, sample_size, sample_half_life, population, pop_penalty, approve, disapprove, net_approve, weight_compare, weight_election, weight_sample, 
...
save(approval_smoothed, file = 'rda/approval_smoothed.rda')
...
approval_ratings %>%
  ...
  geom_ribbon(aes(ymin = smooth_approve - CI_APPROVE$interval[CI_APPROVE$president == PRESIDENT], ymax = smooth_approve + CI_APPROVE$interval[CI_APPROVE$president == PRESIDENT]), color = 'green4', alpha = .15, size = 0) +
  geom_ribbon(aes(ymin = smooth_disapprove - CI_DISAPPROVE$interval[CI_APPROVE$president == PRESIDENT], ymax = smooth_disapprove + CI_DISAPPROVE$interval[CI_APPROVE$president == PRESIDENT]), color = 'red3', alpha = .15, size = 0) +
